...
def get_team(request):
    ...
    ult = query(f"""
    SELECT
    CONCAT(Non_Pemain.Nama_Depan, ' ', Non_Pemain.Nama_Belakang) AS Nama,
    Non_Pemain.Nomor_HP,
    Non_Pemain.Email,
    Non_Pemain.Alamat,
...
FROM
    Pelatih
JOIN
    Non_Pemain ON Pelatih.ID_Pelatih = Non_Pemain.ID
JOIN
    Spesialisasi_Pelatih ON Pelatih.ID_Pelatih = Spesialisasi_Pelatih.ID_Pelatih
WHERE
    Pelatih.Nama_Tim = 'AS Roma'
UNION
SELECT
    NULL,
    Pemain.Nomor_HP,
...
FROM
    Pemain
WHERE
    Pemain.Nama_Tim = 'AS Roma';

    WHERE Nama_Tim='{nama_tim}'
    """)
    
    context = {"pemain_list": res}
