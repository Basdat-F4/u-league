...
def tes_daftar(request):
    ...
    selected_tim_nama = request.POST.get('name')
    selected_univ_nama = request.POST.get('univ')
    id_manajer_result = query(f"SELECT m.id_manajer FROM manajer m WHERE m.Username = '{username}'")
    ...
    with connection.cursor() as cursor:
        ...
        if team_count > 0:
            ...
            print("udh insert tim baru")

            cursor.execute("""
                INSERT INTO tim_manajer (id_manajer, nama_tim)
                VALUES (%s, %s)
            """, [id_manajer, selected_tim_nama])
            print("udh insert manajer ke tim baru")
