    ...
    if request.method == 'POST':
        ...
        with connection.cursor() as cursor:
            ...
            if pelatih_info:
                ...
                cursor.execute("""
                UPDATE Pelatih SET Nama_Tim='{nama_tim}' WHERE Nama_Tim = (SELECT Nama_Tim FROM Pemain 
                               WHERE id_pelatih = '{pelatih_id}')
                               """)
                
                cursor.execute(f"SELECT is_captain FROM Pemain WHERE id_pemain = '{player_id}'")
                cursor.execute(f"UPDATE Pelatih SET Nama_Tim='{nama_tim}' WHERE Nama_Tim = (SELECT Nama_Tim FROM Pemain 
                               WHERE id_pelatih = '{pelatih_id}'")

                # # Insert the pelatih into the team
                # cursor.execute("""
                #     INSERT INTO Pelatih (ID_Pelatih, Nama_Tim)
                #     VALUES (%s, %s)
                # """, [pelatih_id, nama_tim])

                # # Insert the pelatih's spesialisasi
                # cursor.execute("""
                #     INSERT INTO Spesialisasi_Pelatih (ID_Pelatih, Spesialisasi)
                #     VALUES (%s, %s)
                # """, [pelatih_id, spesialisasi])

                # Print a message to indicate a successful insertion
