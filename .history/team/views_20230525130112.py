...
def tes_daftar(request):
    selected_tim_nama = request.POST.get('tim')
    selected_univ_nama = request.POST.get('univ')
    id_manajer_result = query(f"SELECT manajer.id_manajer FROM Tim_Manajer tm JOIN manajer m ON tm.ID_Manajer = m.ID_Manajer WHERE m.Username = '{username}'")
    id_manajer = id_manajer_result[0].id_manajer

    with connection.cursor() as cursor:
        cursor.execute("""
                        INSERT INTO TIM (Nama_Tim, Universitas)
                        VALUES (%s, %s)
                    """, [selected_tim_nama, selected_univ_nama])
        
        cursor.execute("""
                        INSERT INTO TIM_MANAJER (ID_Pelatih, Nama_Tim)
                        VALUES (%s, %s)
                    """, [pelatih_id, nama_tim])
