...
def set_captain(request, player_id):
    with connection.cursor() as cursor:
        cursor.execute(f"SELECT is_captain FROM Pemain WHERE id = {player_id}")
        is_captain = cursor.fetchone()[0]

        if is_captain:
            # If the player is already a captain, remove the captain status
            cursor.execute(f"UPDATE Pemain SET is_captain = false WHERE id = {player_id}")
            message = "Captain position removed"
        else:
            # Assign the player as a captain
            cursor.execute(f"UPDATE Pemain SET is_captain = true WHERE id = {player_id}")
    ...
    return HttpResponseRedirect(reverse("team:get_team"))
