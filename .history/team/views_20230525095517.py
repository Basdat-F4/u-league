...
def insert_pelatih(request):
    ...
    if request.method == 'POST':
        ...
        with connection.cursor() as cursor:
            cursor.execute("SET SEARCH_PATH TO 'u-league'")
            cursor.execute(f"UPDATE Pelatih SET Nama_Tim = '{nama_tim}' WHERE ID_Pelatih IN (SELECT ID FROM Non_Pemain WHERE CONCAT(Nama_Depan, ' ', Nama_Belakang) = '{nama_tim}')")

            # Print a message to check if the update query was executed
            print("Pelatih team updated:", cursor.rowcount)
